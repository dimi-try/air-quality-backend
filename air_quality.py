...
# получаем смещение часового пояса по координатам (в секундах от UTC)
async def get_timezone_offset(lat, lon):
    """
    Получает смещение часового пояса по координатам.
    Использует приближённый расчёт по долготе (15 градусов = 1 час).
    Для более точного определения можно в будущем использовать TimezoneDB API.
    """
    # Приблизительный расчёт: 15 градусов долготы = 1 час = 3600 секунд
